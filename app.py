...
def imprimir_buses(n1, n2, mensaje1="", mensaje2="", meta_color=WHITE):
    ...
    output.extend(generar_bus(n1, "Lopez", f"{RED}{YELLOW}", mensaje1, meta_color if n1 >= 97 else WHITE))
    output.append(115 * "_")  # Línea divisoria entre los dos carriles
    output.extend(generar_bus(n2, "Esmeralda", f"{BLUE}{CYAN}", mensaje2, meta_color if n2 >= 97 else WHITE))
...
def carrera():
    ...
    while a < distance and b < distance:
        ...
        if 7 <= tiempo_actual < 13.5:
            mensaje1 = "TUTUTUTUT (pito del chofer uwu)"
            mensaje2 = "TUTUTUTUT (pito del chofer uwu)"
        elif tiempo_actual >= 13.5 and tiempo_actual < 17:
            mensaje1 = "La SALIDA , LA SALIDAAA"
            mensaje2 = "La SALIDA , LA SALIDAAA"
    ...
    ganador = "Lopez" if a >= distance else "Esmeralda"
    print(f"{GREN}GANÓ LA CARRERA: {ganador}{END}")

# Ejecuta la carrera
carrera()
#Todos los derechos reservados
#Sigueme y te regalo un scooby galleta - :) Instagram: @joshy.zy
# Tik tok joshy.zy
# facebook igual :D 
